...
void getSizes(const List *const books, int *const authorColumnWidth, int *const titleColumnWidth, int *const yearColumnWidth, int *const pageColumnWidth, int *const priceColumnWidth)
{
    ...
    *authorColumnWidth = std::max((size_t)longestAuthorFieldSize(books), strlen("Author"));
    *titleColumnWidth = std::max((size_t)longestBookTitleSize(books), strlen("Title"));
    longestNumberFieldsSize(books, yearColumnWidth, pageColumnWidth, priceColumnWidth);
    *yearColumnWidth = std::max((size_t)*yearColumnWidth, strlen("Pub. year"));
    *pageColumnWidth = std::max((size_t)*pageColumnWidth, strlen("Pages"));
    *priceColumnWidth = std::max((size_t)*priceColumnWidth, strlen("Price"));
}
...
void deleteList(const CLArgs *const args)
{
    ...
    List *newListArr = listCount ? reinterpret_cast<List*>(calloc(listCount, sizeof(List))) : nullptr;
